...
var Config = import_koishi.Schema.intersect([
  import_koishi.Schema.object({
    tool: import_koishi.Schema.object({
      ...
      name: import_koishi.Schema.string().default("google_reverse_image_search").description("工具名称"),
      description: import_koishi.Schema.string().default("使用 Google Lens 进行以图搜图，输入图像 URL，返回包含匹配图片的网页链接").description("工具描述")
    }).description("工具设置")
  }),
  import_koishi.Schema.object({
    provider: import_koishi.Schema.union([
      import_koishi.Schema.const("scrapingdog").description("Scrapingdog"),
      import_koishi.Schema.const("google").description("Google")
    ]).default("scrapingdog").description("服务提供商"),
    apiKey: import_koishi.Schema.string().default("").description("所选服务提供商的 API Key"),
    serverPath: import_koishi.Schema.string().default("").description("Koishi 在公网中的地址，用于替换查询中 `chatluna-storage-service` 的内网 `serverPath` 地址，使得 Google Lens 可以访问、将其用于查询；若 `chatluna-storage-service` 中填写的 `serverPath` 就是公网地址，请留空此处"),
    timeoutSeconds: import_koishi.Schema.number().default(20).min(5).max(120).description("网络请求超时（秒）"),
    maxResults: import_koishi.Schema.number().default(10).min(1).max(50).description("最大返回结果数"),
    customPrompt: import_koishi.Schema.string().role("textarea").default(`以下网站更可能是作者发布原图/包含指向原帖的链接的地方（按优先级从上至下排序）：
x.com
danbooru.donmai.us
各种rule34
bsky.app
...
请勿尝试使用其他工具查询图像来源！`).description("将你自己的以图搜图经验或输出要求告知模型，如：在哪些网页更加可能找到原图、以什么格式给出包含匹配图片的网页链接（在此处填写的提示词将会附加在正常的工具响应中以指导模型）"),
  ...
  import_koishi.Schema.object({
    cacheService: import_koishi.Schema.object({
      enable: import_koishi.Schema.boolean().default(true).description("启用同 URL 结果缓存"),
      ttlSeconds: import_koishi.Schema.number().default(24 * 60 * 60).min(60).description("缓存有效期（秒）"),
      cleanupIntervalSeconds: import_koishi.Schema.number().default(10 * 60).min(60).description("过期缓存清理间隔（秒）"),
      cacheThumbnails: import_koishi.Schema.boolean().default(true).description("缓存 Scrapingdog 返回的缩略图到 `chatluna-storage-service`，以便模型可以读取（需要安装并启用 `koishi-plugin-chatluna-storage-service`）")
    }).description("缓存设置")
  })
]);
...
var schema = import_zod.z.object({
  imageUrl: import_zod.z.string().url().describe("需要执行以图搜图的图像 URL。")
});
var GoogleReverseImageTool = class extends import_tools.StructuredTool {
